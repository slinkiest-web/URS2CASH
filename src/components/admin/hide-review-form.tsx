...
import { hideReview } from "@/lib/actions/admin";
import { Button } from "@/components/ui/button";

/** HARD RULE (§11.2): hideReview sets is_hidden only — never score/rating_average/rating_count. */
export function HideReviewForm({ ratingId }: { ratingId: string }) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await hideReview(ratingId, reason);
      if (!result.ok) {
