...
import { getOrdersAsBuyer } from "@/lib/orders/list-orders";
...
/** Server Component (PRD §5.3). The buyer's own purchases — minimal list, links into /orders/[id]. */
export default async function BuyerOrdersPage() {
  ...
  const orders = user ? await getOrdersAsBuyer(supabase, user.id) : [];
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-6 py-10">
      ...
      {orders.length > 0 ? (
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-400">You haven&apos;t bought anything yet.</p>
      )}
    </main>
  );
}
