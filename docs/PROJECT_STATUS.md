...
**As of:** 2026-07-29 (Prompt 14 session)
**Sources:** `docs/urs2cash-prd.md` (v2.0, locked), `docs/HANDOFF.md` (Prompts 1–14, plus two un-numbered sessions), `docs/DECISIONS.md` (#1–60), `docs/KNOWN_ISSUES.md` (#1–29, several resolved/superseded by Prompts 11–14), repo state pending commit on `main` at the time of writing (branch was at `5002453`, this session's changes committed after this report).
**Verified for this report:** `npm run typecheck`, scoped `eslint "src/**/*.{ts,tsx}"`, `npm run test` (122/122), `npm run build` all clean. One new migration this prompt (`20260729100000_mark_order_paid_function.sql`), applied directly against the local Postgres instance and its access-control restriction (the actual security boundary for this prompt) confirmed live via a rejected `anon`-role RPC call, not just read from the SQL. Full webhook flow — signature verification, idempotency, amount/reference reconciliation, the atomic paid+sold transition, event firing — live-verified against the running dev server and local Postgres using a real `sk_test_...` key and hand-signed payloads (no tunnel needed). See `docs/HANDOFF.md`'s Prompt 14 entry for the full verification log.

**Commit state:** as of the start of this session, `main` was fully committed and pushed at `5002453`. This report is written before Prompt 14's own commit — see `git log` for the actual current HEAD.
...
Fourteen "prompts" (build sessions) have shipped so far, each logged in full in `docs/HANDOFF.md`, plus two un-numbered sessions: one between Prompts 8 and 9 that fixed a critical schema-access bug (§3 below), and one after Prompt 11 that ran a full real-browser QA pass over the buyer-facing surface (§3.5). This document summarizes where that leaves the codebase.
...
| **D1 — Checkout** | **Built up to Paystack initialize (Prompt 13)** | `initiateCheckout` server action: auth + verified-email + self-purchase checks, Zod-validated delivery details, money snapshot (`amount_kobo`/`commission_kobo`=floor(10%)/`seller_payout_kobo`), service-role `pending` order insert (RLS has no `authenticated` INSERT policy on `orders` at all — by design), the concurrency race (AC8) caught via a new partial-unique-index constraint violation, not a pre-check, Paystack `initialize` called server-side. Buy flow lives on `/l/[id]`. **Not built:** `/orders/[id]` callback page (AC7), the 30-minute expiry cron (AC9, issue #25). |
| **D2 — Payment confirmation** | **Built (Prompt 14)** | `/api/webhooks/paystack` — the only code that may transition an order to `paid`, enforced structurally (the atomic `mark_order_paid` Postgres function has `EXECUTE` revoked from every role but `service_role`, confirmed live via a rejected `anon` RPC call). Signature verified against the raw body (HMAC-SHA512, `PAYSTACK_SECRET_KEY` — Paystack has no separate webhook secret, Decision #57), idempotency via `webhook_events` UNIQUE(provider, event_id) insert-first, amount + reference reconciliation (mismatch flags via `processed_at IS NULL`, never transitions), `order_paid`/`contact_details_released` fired. **Not built:** actual email sending (AC6, issue #28 — no transactional email infra exists anywhere yet). |
| **D3–D6 — Fulfilment, delivery/release, dispute, rating** | **Not built** | No order-state-machine actions past `paid` (ship, deliver, release, dispute, rate). Schema + triggers already exist (Prompt 5/6). |
...
**Net:** the database schema is complete for the whole PRD and, as of the un-numbered session before Prompt 9, **actually verified against a live Postgres instance** — Prompts 13 and 14 each added one more real schema fix on top of it (Decisions #54, #60). The application layer now covers **Epic A (partial), Epic B (full except price guidance), moderation detection, all of Epic C** (browse, search, listing detail, seller public profile), **and D1–D2 of Epic D** (checkout through a confirmed `paid` transition — an order can now genuinely become `paid` and its listing `sold`, for real, end to end). **Epic E has zero application code.** The rest of the order state machine (ship, deliver, release, dispute, rate) is next.

---

## 3. Database schema — now live-verified, not just written

Nine migrations exist. Every one has been applied via `npx supabase db reset` (or, for the newest two, applied directly against the running local instance and confirmed identical) against a real local Postgres instance and spot-verified by direct query/trigger execution — this closes what was previously the single largest cross-cutting risk in the project (Known Issues #1/#2/#8/#13/#16/#17/#18, all now resolved or superseded).
...
- **Public-column-privacy pattern:** a table with RLS that can't express "hide column X but keep column Y public" gets a dedicated `_public` view instead. Applied to `profiles` and `ratings`. **Not yet applied to `orders`** for buyer delivery details pre-`paid` (Known Issue #14, still open — genuinely live as of Prompt 14, since the webhook now creates real `paid` orders with real delivery data sitting behind it).
...
3. **No read-side projection hides buyer delivery details on `orders` pre-`paid`** (issue #14) — genuinely live now, not theoretical: Prompt 14's webhook creates real `paid` orders with real delivery data sitting behind `orders_select_participant`'s unrestricted base-table policy. Should be closed as part of the very next order-detail read surface (Prompt 15), not deferred again.
4. **No scheduled job expires stale `pending` orders** (issue #25, §10 Epic D1 AC9) — `/api/cron/expire-pending-orders` is still the empty Prompt-1 scaffold.
5. **`/orders/[id]` (the Paystack callback destination) doesn't exist** (issue #26) — referenced by URL from `initiateCheckout`, not built.
6. **No transactional (non-auth) email sending exists anywhere** (issue #28, §10 Epic D2 AC6) — `order_paid`/`contact_details_released` fire as analytics events only; no email is actually sent yet. Flagged since Prompt 2 (Decision #4), still not built.
7. **Amount/reference-mismatch admin flag has no dedicated table or UI** (issue #29, Decision #58) — surfaced only via `webhook_events.processed_at IS NULL`, no notification, no admin view (Epic E doesn't exist).
8. **No `view_count` column** (issue #22) — Epic B4 AC1 asks the dashboard to show it; doesn't exist in the schema.
9. **`/dashboard/listings` still has no inbound nav link** (issue #24) — a global nav now exists (Prompt 10) but is buyer-facing only; needs a seller-facing link added.
10. **Dynamic listing form only reveals `usageIndicatorFields` conditionally** (issue #20) — other conditionally-required fields always render once a category is picked. Server-side enforcement is complete; UX-only gap.
11. **Numeric attribute range filtering is out of scope on category pages** (Decision #42) — not a defect, a documented scope boundary tied to the current GIN index shape.
12. **No edge caching (`revalidate`) on any discovery/detail page** — `createClient()`'s use of `cookies()` forces dynamic rendering regardless of any `revalidate` export; genuine caching would need a separate non-cookie-bound anon client, a real architecture change not taken on as a side effect of Prompt 10 or 11.
13. **A seller can preview her own `draft` listing at `/l/[id]`** (Prompt 11) — an incidental consequence of `listings_select_own` RLS letting an owner read any status of her own row. Not harmful (no purchase path exists to expose), not asked for, just worth knowing.
14. **Per-listing hygiene notices (e.g. flagging Beauty's hygiene-sensitive product types specifically) were not built** (Prompt 11, Decision #49) — narrowed to what's registry-derivable; Personal Care's category-wide "used disallowed" policy is already structurally visible via its condition selector.
15. **No live test of a successful Paystack `initialize` → real hosted payment → real webhook delivery over the public internet** (issue #27, remaining half) — needs a tunnel (ngrok) or a real completed test-mode payment; not available in this environment.
16. **Real-uploaded-photo Lighthouse LCP still hasn't been measured** — both Prompt 11's own Lighthouse run and the QA session's screenshots used seed data with placeholder (never-uploaded) Storage URLs. Structural mitigations (`next/image` format negotiation, `priority` on the first gallery photo) are in place, but an end-to-end run through the real `/sell` upload flow with a genuine photo hasn't been done.

---

## 6. Recommended next steps

Epic C (browse, search, listing detail, seller public profile) is fully built and live-verified end to end (Prompt 12). Epic D1–D2 (checkout through a confirmed `paid` transition) is built and live-verified end to end (Prompts 13–14) — a real order can now become `paid`, its listing genuinely flips to `sold`, and the whole flow is provably idempotent and signature-verified. Nothing marks an order `paid` except the webhook.

In dependency order:

1. **The rest of the order state machine — the immediate next prompt.** `markShipped` (sets `shipped_at`, `auto_release_at = shipped_at + 7 days`, requires a `tracking_note` ≥ 3 chars, §10 Epic D3 AC2), `confirmDelivery` (buyer-triggered, immediately cascades to `released` + creates the `payouts` row, §10 Epic D4 AC1–AC3), `cancelOrder` (seller-triggered while `paid` and not yet `shipped`, §10 Epic D3 AC6). This is also the natural point to build the first seller/buyer-facing order-detail read surface — and it should close Known Issue #14 as part of its own design (an `orders_participant_view`, same pattern as `profiles_public`/`ratings_public`), not as an afterthought.
2. **The cron-driven auto-transitions** — 30-minute `pending` → `expired` (issue #25, AC9) and 7-day `shipped` → auto-`released` (§10 Epic D4 AC5). Both are scheduled jobs at `/api/cron/*`, currently empty scaffolding.
3. **`/orders/[id]`** (the Paystack callback page, issue #26, §10 Epic D2 AC7) — reads status, writes nothing. Overdue; `initiateCheckout` has referenced this URL since Prompt 13.
4. **Admin-role mechanism** (Known Issue #12) — needed before any Epic E work; land it as its own deliberate migration.
...
Rating-review contact-detail scanning (§7.1 HARD RULE, Epic D6) has a documented TODO in `src/lib/moderation/contact-detector.ts`'s module docstring specifying exactly how to wire it once `submitRating`/`src/lib/actions/ratings.ts` exists — don't rebuild this from scratch when Epic D6 lands. Transactional email (issue #28) and the amount-mismatch admin flag (issue #29) are both real gaps but don't block the state-machine work — pick them up when Epic D3–D6 or Epic E actually needs them.
